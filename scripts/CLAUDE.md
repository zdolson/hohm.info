...
- `lib/browser.ts` — Singleton stealth Chromium browser with retry, jitter, and challenge detection. Quick URL/title block check after navigation before long waits. Exports `fetchPageHtml()`, `closeBrowser()`.
...
- `lib/fetchers/trulia.ts` — Full Trulia HTML scraper (cheerio) + `TruliaAdapter`. Throws typed `FetchError`. No API key needed; stability: fragile. Higher photo cap + hero/og fallbacks when gallery array empty.
...
- `lib/llm/flatten-schema.ts` — `flattenSchemaForGemini()` inlines `$ref`/`definitions` for Gemini JSON schema (API rejects bare refs).
- `lib/llm/prompt.ts` — `buildSystemPrompt`, `buildUserPrompt`, `PROMPT_VERSION`.
- `lib/llm/gemini.ts` — `GeminiProvider`; passes flattened schema to `responseJsonSchema`.
- `lib/llm/batch.ts` — Gemini-only `inferTagsBatch(config, items)` (Batch API, 50% cost); 20MB inline limit; `BatchInferenceItem`, polling until terminal state; flattened schema; higher default `maxOutputTokens`; elapsed logging while polling.
...
- `lib/ai-tag-inference.ts` — `InferenceInput`, `computeInputFingerprint` (SHA-256 of normalized input), `inferTags(provider, input, opts)`; `LLM_MAX_PHOTOS` read at runtime; debug saves all photos to disk but only first N to LLM; optional `llm-response-raw.txt` on parse failure hints; default `maxTokens` 8192; when `opts.debug` is true (or CLI `--debug`), writes to current address debug dir (see `lib/debug.ts`): `prompt.txt`, `photos/photo-0.jpg`, … Logs elapsed timestamps at inference stages.
